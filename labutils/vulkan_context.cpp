...
namespace
{
	...
	std::optional<std::uint32_t> find_compute_queue_family(VkPhysicalDevice aPhysicalDev);
...
#		endif // ~ debug builds
		...
		// Load a Compute queue families
		if( auto const index = find_compute_queue_family( ret.physicalDevice ) )
		{
			ret.computeFamilyIndex = *index;
		}
		else
		{
			throw lut::Error( "No queue family with COMPUTE" );
		}
		...
		vkGetDeviceQueue( ret.device, ret.computeFamilyIndex, 0, &ret.computeQueue );
